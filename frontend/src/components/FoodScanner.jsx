...
const FoodScanner = () => {
  ...
  const handleLog = async () => {
    ...
    try {
      ...
      // 1. Format the data to match your backend's foodItemSchema
      const nutritionData = [
        {
          name: result.foodName,
          calories: result.calories,
          serving_size_g: result.portionSize, // Backend maps this to 'serving'
          protein_g: result.protein, // Backend maps this to 'protein'
          carbohydrates_total_g: result.carbs, // Backend maps this to 'carbs'
          fat_total_g: result.fat,
          sugar_g: result.sugar || 0,
          cholesterol_mg: 0, // Optional fallback if AI doesn't return it
        },
      ];
      ...
      const res = await fetch("http://localhost:5000/api/meals/register", {
        ...
        body: JSON.stringify({
          mealType: "lunch", // Defaulting to lunch
          query: `Scanned: ${result.foodName}`,
          nutritionData: nutritionData,
        }),
      });

      if (!res.ok) throw new Error("Failed to save scanned meal to diary");

      // 3. Navigate to diary on success
      navigate("/food-diary"); // Note: Make sure this matches your actual route (e.g. '/diary' or '/food-diary')
    } catch (err) {
      console.error(err);
      setError("Could not save meal to database. Please try again.");
    } finally {
      setIsLogging(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 sm:px-8 space-y-8 pb-24">
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={() => navigate(-1)}
          className="p-2 bg-white rounded-xl shadow-sm border border-slate-100 text-slate-500 hover:text-emerald-600 transition-colors"
        >
          <ArrowLeft size={24} />
        </button>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-white space-y-6 bg-slate-800">
            ...
            <div className="text-center space-y-2 px-4">
              ...
              <p className="text-slate-400 text-sm max-w-[200px] mx-auto">
                Point your camera at your meal for instant nutrient breakdown.
              </p>
            </div>
            <div className="flex gap-4">
              <button
                onClick={startCamera}
                className="bg-emerald-600 px-6 py-4 rounded-2xl font-bold hover:bg-emerald-700 transition-all flex items-center gap-2"
              >
                <Camera size={20} /> Open Camera
              </button>
              <button
                onClick={() => fileInputRef.current?.click()}
                className="bg-white/10 px-6 py-4 rounded-2xl font-bold hover:bg-white/20 transition-all flex items-center gap-2 border border-white/10"
              >
        ...
        {isCameraActive && (
          <>
            ...
            <div className="absolute bottom-6 left-0 right-0 flex items-center justify-center gap-8">
              <button
                onClick={stopCamera}
                className="p-4 bg-white/20 backdrop-blur-xl text-white rounded-full"
              >
                <RefreshCw size={24} />
              </button>
              <button
                onClick={capturePhoto}
                className="w-20 h-20 bg-white rounded-full border-4 border-emerald-500/30 flex items-center justify-center hover:scale-105 transition-transform"
              />
            </div>
          </>
        )}
        ...
        {image && (
          <div className="relative w-full h-full">
            ...
            {!isAnalyzing && !error && (
              <button
                onClick={handleReset}
                className="absolute top-4 right-4 p-3 bg-black/50 backdrop-blur-md text-white rounded-full z-10 hover:bg-black/70 transition-all"
              >
                <RefreshCw size={20} />
              </button>
            )}
        ...
        <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-xl space-y-6 animate-in fade-in slide-in-from-bottom-10">
          <div className="flex items-center justify-between border-b border-slate-50 pb-6">
            ...
            <div className="text-right whitespace-nowrap pl-2">
              ...
              <span className="text-xs font-bold text-slate-400 uppercase ml-1 block">
                kcal
              </span>
            </div>
          </div>
          ...
          <button 
            onClick={handleLog} 
            disabled={isLogging}
            className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black text-lg shadow-xl hover:bg-emerald-700 transition-all active:scale-95 disabled:opacity-70 disabled:hover:scale-100 flex justify-center items-center gap-2"
          >
            {isLogging ? <Loader2 size={24} className="animate-spin" /> : "Log This Meal"}
          </button>
        </div>
      )}
